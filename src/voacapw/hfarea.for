...
c jw         if(iy.eq.1) then
c jw            write(alf,'(i3,'' rows['',i2)') ny,iy
c jw         else
c jw            write(alf,'(i3)') iy
c jw         end if
c jw         call soua@(alf)
c jw         if(mod(iy,20).eq.0) then
c jw            call sou@(' ')            !  cause a <new line>
c jw            call coua@('        ')    !  space over to line things up
c jw         end if
c jw       end if
      if(iquiet.eq.0) then
         if(iy.eq.1) then
...
c jw      doesit=fexists@(run_directory(1:nch_run)//'\voaarea.abt',error_code4)
c jw      if(doesit) then      !  file exists, abort area calculations
c jw         call erase@(run_directory(1:nch_run)//'\voaarea.abt', error_code)   !  delete file first
c jw         call unlink(run_directory(1:nch_run)//'\voaarea.abt', error_code)   !  delete file first
c jw         return 1
c jw      end if
      if(icancel_batch.ne.0) return     !  other cancel button picked
      DO 900 IX=1,NX
c jw      call yieldit    !  yield for Windows
...
C.......FORCE LONG PATH MODEL IF PATH LENGTH .GT. SPECIFIED LIMIT
        IPFG=100
        IF(MSPEC.ne.121 .and. GCDKM.GT.GCDLNG)IPFG=200
        IF(GCDKM.GT.GCDLNG)IPFG=200
      ENDIF
